...
def get_fore_back(img):
    ...
    Args:
    -----
        img: The input image can either be of type uint8 with range [0,255] or float with range[0, 1]
    ...
    """
    if img.dtype != np.uint8 and np.max(img) <= 1.0 and np.min(img) >= 0:
        img = np.uint8(255 * img)
    elif img.dtype == np.uint8 and np.max(img) <= 255 and np.min(img) >= 0:
        img = img.copy()
    else:
        raise ValueError("Input image must be either dtype uint8 with range [0, 255] or float with range [0, 1] but got img.dtype = {}".format(img.dtype))
